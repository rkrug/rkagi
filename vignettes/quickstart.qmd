...
use `add_sbstract_to_parquet()` to call Summarize for Search rows and write
search parquet with an `Abstract` column.

```r
out_search_with_abs <- add_sbstract_to_parquet(
  connection = conn,
  input_parquet = parquet_dir,
  output = "search_with_abstract",
  overwrite = TRUE
)

# parquet output now includes Abstract for downstream vector workflows
```
