# kagiPro Corpus Workflow: Search to Abstract-Linked Corpus

## End-to-End Corpus Workflow (Multiple Queries)

This vignette shows one complete, reproducible workflow focused on the
Search endpoint:
...
The goal is to create a corpus that is ready for downstream
vector/comparison workflows while preserving query-level partitions.
...
Using a named list keeps query names stable throughout
JSON/parquet/content partitions (`query=<query_name>`).
...
convert JSON to parquet explicitly:
...
At this stage, `tbl` is a query-partitioned search corpus with an
additional `abstract` column, ready for downstream modeling and
...
- [`download_content()`](https://rkrug.github.io/kagiPro/reference/download_content.md),
  ...
  all support selector expansion (`endpoint = NULL` and/or
  `query_name = NULL`).
- `read_corpus(abstracts = TRUE)` expects abstract parquet schema with
  lowercase `abstract`.
